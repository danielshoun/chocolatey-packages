...
$nuspecPath = Join-Path $packageDir 'zed-editor.nuspec'
$installScriptPath = Join-Path $packageDir 'tools\chocolateyinstall.ps1'
$releaseApiUrl = 'https://api.github.com/repos/zed-industries/zed/releases/latest'
$windowsAssetPattern = '^Zed-x86_64\.exe$'
...
function Get-GitHubHeaders {
    $headers = @{
        ...
        'User-Agent' = 'chocolatey-packages-zed-editor-updater'
...
if ($Repack) {
    ...
    Write-Host "Repacking Zed package at current version $currentVersion."
...
if (-not $asset) {
    throw 'Could not find the Windows x64 installer in the latest Zed release.'
}

if (-not $Force -and $currentVersionInfo.BaseVersion -eq $latestVersion) {
    Write-Host "Zed is already up to date at version $currentVersion."
    return
}
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("zed-choco-update-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Write-Host "Updated Zed package files to version $packageVersion."
